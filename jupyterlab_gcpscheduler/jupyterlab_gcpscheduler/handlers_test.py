...
class ProjectHandlerTest(tornado.testing.AsyncHTTPTestCase):

  URL = PATH + '/project'

  def get_app(self):
    return Application([
        (self.URL, handlers.ProjectHandler),
    ])

  def test_get(self, mock_auth_provider):
    mock_provider = MagicMock()
    mock_provider.project = TEST_PROJECT
    mock_auth_provider.get.return_value = mock_provider
    response = self.fetch(self.URL)
    self.assertEqual(b'{"project": "test-project"}', response.body)

  def test_get_throws_error(self, mock_auth_provider):
    mock_auth_provider.get.side_effect = RefreshError('unable to get token')
    response = self.fetch(self.URL)
    self.assertEqual(403, response.code)
    self.assertEqual('Unable to determine Google Cloud Project',
                     response.reason)
...
class ProxyHandlerTest(tornado.testing.AsyncHTTPTestCase):
  ...
  def test_get(self, mock_auth_provider, mock_client):
    ...
    self.assertEqual('jupyterlab_gcpscheduler/{}'.format(version.VERSION),
                     request.user_agent)
...
class RuntimeEnvHandlerTest(tornado.testing.AsyncHTTPTestCase):
  ...
  def get_app(self):
    return Application([
        (self.URL, handlers.RuntimeEnvHandler),
    ])

  def tearDown(self):
    if os.environ.get(handlers.FRAMEWORK_ENV_VAR):
      del os.environ[handlers.FRAMEWORK_ENV_VAR]

  def test_get(self):
    os.environ[handlers.FRAMEWORK_ENV_VAR] = self.tempfile
    response = self.fetch(self.URL)
    self.assertEqual(b'Tensorflow', response.body)

  def test_get_no_envar(self):
    response = self.fetch(self.URL)
    self.assertEqual(b'unknown', response.body)

  def test_get_no_file(self):
    os.environ[handlers.FRAMEWORK_ENV_VAR] = '/path/to/junk/file'
    response = self.fetch(self.URL)
    self.assertEqual(b'unknown', response.body)
