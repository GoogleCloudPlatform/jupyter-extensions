...
class BranchHandler(APIHandler):
  ...
  def create_branch(self, path, branch):
    ...
    ) == 0, "Branch {} could not be created. Please check that the branch name does not exist.".format(
        branch)

  def change_branch(self, path, branch):
    assert subprocess.call(
        ['git', 'checkout', branch], cwd=path
    ) == 0, "Could not switch to branch {}. Please commit or stash your changes and try again. ".format(
        branch)
...
class SyncHandler(APIHandler):
  ...
  * uses git-sync-changes bash script
  ...
  def sync_repo(self, path, options):
    res = subprocess.run(['git', 'sync-changes'] + options,
                         cwd=path,
                         capture_output=True)
