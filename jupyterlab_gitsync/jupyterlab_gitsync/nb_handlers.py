...
class NotebookInitHandler(APIHandler):
  ...
  * creates dir in '.sync_cache' to hold file caches
  ...
  def init_cache_files(self, path, fpath, dpath):
    ...
    """
    assert subprocess.call(['cp', fpath, dpath + '/base.ipynb'],
                           cwd=path) == 0, 'init base.ipynb failed'
    assert subprocess.call(['cp', fpath, dpath + '/local.ipynb'],
                           cwd=path) == 0, 'init local.ipynb failed'
    assert subprocess.call(['cp', fpath, dpath + '/remote.ipynb'],
                           cwd=path) == 0, 'init remote.ipynb failed'
    assert subprocess.call(['cp', fpath, dpath + '/merged.ipynb'],
                           cwd=path) == 0, 'init merged.ipynb failed'
  ...
  * if merge successful: update disk version to merged version
  ...
  def update_base(self, path, dpath):
    assert subprocess.call(
        ['cp', dpath + '/merged.ipynb', dpath + '/base.ipynb'],
        cwd=path) == 0, 'update base.ipynb failed'
  ...
  def update_remote(self, path, fpath, dpath):
    assert subprocess.call(['cp', fpath, dpath + '/remote.ipynb'],
                           cwd=path) == 0, 'update remote.ipynb failed'
  ...
  def merge_notebooks(self, path, dpath):
    ...
    if (res == b''):
      return True
        ...
        res == b'[W nbmergeapp:57] Conflicts occured during merge operation.'):
      return False
    else:
      raise Exception(
          'Nbmerge was unable to process files. Make sure files are .ipynb files.'
      )

  def update_disk_file(self, path, fpath, dpath):
    assert subprocess.call(['cp', dpath + '/merged.ipynb', fpath],
                           cwd=path) == 0, 'update {} failed'.format(fpath)
...
class NotebookResolveHandler(APIHandler):
  ...
  def update_disk_file(self, path, fpath, dpath, origin):
    ...
    assert subprocess.call(
        ['cp', copy_file_path, fpath],
        cwd=path) == 0, 'update {} after conflict failed'.format(fpath)
