...
function getDiffOps(diff) {
  ...
  const diff_arr = diff.flatMap(x => {
    const xmap = [];
    if (x.buffer1 && x.buffer1.length) {
      xmap.push({ op: 'removed', value: x.buffer1.join('\n') + '\n' });
    }
    if (x.buffer2 && x.buffer2.length) {
      xmap.push({ op: 'added', value: x.buffer2.join('\n') + '\n' });
    }
    if (x.common) {
      xmap.push({ op: 'unmodified', value: x.common.join('\n') + '\n' });
    }
    return xmap;
  });

  for (let i = 0; i < diff_arr.length; i++) {
    const cells = diff_arr[i].value.split(CELL_DIVIDER + '\n');
...
function mapMergedCells(base_str, local_str, remote_str, merged_str) {
  const local_diff = diff3.diffComm(
    base_str.split('\n'),
    local_str.split('\n')
  );
  const remote_diff = diff3.diffComm(
    base_str.split('\n'),
    remote_str.split('\n')
  );
  const merged_diff = diff3.diffComm(
    base_str.split('\n'),
    merged_str.split('\n')
  );
  ...
  while (m < merged_ops.length) {
    switch (merged_ops[m]) {
      ...
      case 'modified':
        ...
        if (modifier === 'local' && remote_ops[r] === 'modified') {
          modifier = 'both';
        }
...
function mergeCellMetadata(base, local, remote) {
  ...
  if (_.isEqual(local_meta, base_meta) && _.isEqual(remote_meta, base_meta)) {
    ...
    _.isEqual(local_meta, base_meta) &&
    !_.isEqual(remote_meta, base_meta)
  ) {
    return remote_meta;
  } else if (
    !_.isEqual(local_meta, base_meta) &&
  ...
  source.forEach((value, index) => {
    switch (value) {
      ...
      case '\n>>>>>>>>>\n':
        ...
        if (
          cindex.cell &&
          cindex.local !== undefined &&
          cindex.divider &&
          cindex.remote
        ) {
          source[cindex.local] =
            '<span style="color:red"><<<<<<<<< LOCAL</span>' + CELL_DIVIDER;
          source[cindex.divider] =
            '<span style="color:red">=========</span>' + CELL_DIVIDER;
          source[cindex.remote] =
            '<span style="color:red">>>>>>>>>> REMOTE</span>' + CELL_DIVIDER;
        }
        break;
      case CELL_DIVIDER:
        cindex.cell = true;
...
function mergeCells(base, local, remote) {
  ...
  const result = diff3.merge(local_str, base_str, remote_str, {
    stringSeparator: '\n',
  });
  if (result.conflict) {
  ...
  while (i < mapping.length) {
    if (mapping[i].op === null) {
      const cell = mergeCellMetadata(
        base.cells[b],
        local.cells[l],
        remote.cells[r]
      );
      cell.source = merged_source[m]
        .substr(0, merged_source[m].length - 1)
        .match(/[^\n]+\n?|\n/g);
      cells.push(cell);
      ...
      if (mapping[i].modifier === 'local') {
        b++;
        r++;
      }
      if (mapping[i].modifier === 'remote') {
        b++;
        l++;
      }
