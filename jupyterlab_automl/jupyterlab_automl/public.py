...
from jupyterlab_automl.types import ModelFramework
...
def create_dataset(display_name: str,
  ...
  if dataframe is not None:
    return AutoMLService.get().create_dataset_from_dataframe(
...
def export_dataset(dataset_id: str) -> pandas.DataFrame:
  """Export an existing tables dataset to a Pandas DataFrame

  Args:
      dataset_id (str): ID of the dataset to export
...
def import_saved_model(display_name: str, model_path: str,
  ...
  Args:
      ...
      framework (jupyterlab_automl.types.ModelFramework): The framework used to train the model
