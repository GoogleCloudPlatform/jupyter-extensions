...
import { Context } from '../components/automl_widget';
...
df = export_dataset('${datasetId}')`;
  ...
  static exportModelCode(path: string, displayName: string, framework: string) {
    return `from jupyterlab_automl import import_saved_model, ModelFramework
...
model = import_saved_model(display_name="${displayName}",
