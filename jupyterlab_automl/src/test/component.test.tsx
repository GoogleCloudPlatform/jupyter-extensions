...
import { ImportData } from '../components/datasets/import_data';
...
describe('Rendering of Import Data', () => {
  const importData = shallow(
    <ImportData open={true} onClose={null} onSuccess={null} context={null} />
  );
  it('Renders initial state correctly', async () => {
    expect(importData).toMatchSnapshot();
    expect(importData.state('from')).toEqual('computer');
    expect(importData.state('name')).toEqual('');
  });
  it('Updates name state based on input', async () => {
    const nameInput = importData.find(TextInput);
    nameInput.simulate('change', { target: { value: 'my_dataset' } });
    expect(importData.state('name')).toEqual('my_dataset');
  });
  it('Updates from state based on radio toggle', async () => {
    const radioInput = importData.find(RadioInput);
    radioInput.simulate('change', { target: { value: 'bigquery' } });
    expect(importData.state('from')).toEqual('bigquery');
    expect(importData.state('source')).toEqual(null);
    expect(importData.state('error')).toEqual(null);
  });
  it('Catches incorrectly formatted URIs', async () => {
    const bigQueryUri = importData.find(TextInput).at(1);
    bigQueryUri.simulate('change', { target: { value: 'invalid uri' } });
    expect(importData.state('source')).toEqual('invalid uri');
    expect(importData.state('error')).toEqual('Invalid BigQuery uri');
  });
});

describe('Rendering of ListResourcePanel', () => {
  ...
  it('Renders initial state correctly', async () => {
    ...
    expect(listResourcesPanel.state('importDatasetDialogOpen')).toEqual(false);
    expect(listResourcesPanel.state('importModelDialogOpen')).toEqual(false);
  });
  it('Create button opens import dataset dialog', async () => {
    const createButton = listResourcesPanel.find(Button);
    createButton.simulate('click');
    expect(listResourcesPanel.state('importDatasetDialogOpen')).toEqual(true);
  });
  it('Toggling to models changed the resource type and toolbar button', async () => {
    ...
    expect(listResourcesPanel.state('importModelDialogOpen')).toEqual(true);
  });
...
describe('Rendering of Copy Code Component', () => {
