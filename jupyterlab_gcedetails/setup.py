...
tar_path = pjoin(here, 'jupyterlab_gcedetails*.tgz')
...
LONG_DESCRIPTION = 'Google Compute Engine VM Details extension for JupyterLab'
...
class NPM(Command):
  ...
  def run(self):
    ...
    if self.should_run_npm_pack():
      log.info('Packing jupyterlab_gcedetails into archive')
      check_call(['npm', 'pack'],
                 cwd=here,
                 stdout=sys.stdout,
                 stderr=sys.stderr)
