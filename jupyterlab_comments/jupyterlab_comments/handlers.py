  ...
  def get(self):
    try:
      ...
      if git.inside_git_repo(full_file_path_dir):
        ...
        file_path_from_repo_root = os.path.relpath(full_file_path,
                                                   start=git_root_dir)
        comments = git.get_comments_for_path(file_path_from_repo_root,
                                             git_root_dir)
        ...
        self.finish(
            json.dumps({
                "error_message": "Error: file is not inside a git repository"
            }))
  ...
  def get(self):
    try:
      ...
      if git.inside_git_repo(full_file_path_dir):
        ...
        file_path_from_repo_root = os.path.relpath(full_file_path,
                                                   start=git_root_dir)
        comments = git.get_all_code_review_comments(file_path_from_repo_root,
                                                    git_root_dir)
        if comments is None:
...
class AddDetachedCommentHandler(APIHandler):
  ...
  def post(self):
    try:
      ...
      line_num = data.get(
          'line',
          0)  #startLine = 0 means comment not attached to any line in the file
      full_file_path = os.path.join(server_root, file_path)
      full_file_path_dir = os.path.dirname(full_file_path)
      git_root_dir = git.get_repo_root(full_file_path_dir)
      file_path_from_repo_root = os.path.relpath(full_file_path,
      ...
      if parent:
        git.add_detached_reply_comment(file_path_from_repo_root, git_root_dir,
                                       comment, parent)
      else:
        git.add_detached_comment(file_path_from_repo_root, git_root_dir,
                                 comment, line_num)
...
class AddReviewCommentHandler(APIHandler):
  ...
  def post(self):
    try:
      ...
      if parent:
        git.add_review_reply_comment(file_path_from_repo_root, git_root_dir,
                                     comment, parent, review_hash)
      else:
        git.add_review_comment(file_path_from_repo_root, git_root_dir, comment,
                               review_hash, line_num)
...
class RefreshIntervalHandler(APIHandler):
  ...
  def get(self):
    try:
      ...
      print(
          "Error fetching configured refresh interval traitlet, JSON not serializable"
      )
      print(traceback.format_exc())
  ...
  def post(self):
    try:
      ...
      if git.inside_git_repo(full_file_path_dir):
        ...
        print(
            "Error: file is not inside a git repository, cannot pull from remote"
        )
        self.finish()
    except Exception as e:
