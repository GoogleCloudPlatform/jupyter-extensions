...
def setup_repos(working_dir):
    ...
    subprocess.call(['git', 'init'], cwd=remote_dir, stdout=FNULL, stderr=subprocess.STDOUT)
    create_files(1, remote_dir)
    create_files(1, not_git_dir)
    subprocess.call(['git', 'add', '.'], cwd=remote_dir, stdout=FNULL, stderr=subprocess.STDOUT)
    subprocess.call(['git', 'commit', '-a', '-m', '"Initial commit with a test file"'], cwd=remote_dir, stdout=FNULL, stderr=subprocess.STDOUT)
...
def add_review_comment(text, file, id_hash, cwd):
    subprocess.call(['git', 'appraise', 'comment', '-f', file, '-m', text, id_hash], cwd=cwd, stdout=FNULL, stderr=subprocess.STDOUT)
